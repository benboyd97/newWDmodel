...
def main():
    ...
    for i, pb in enumerate(mag_table):
        ...
        with pm.Model() as hierarchical:
            ...
            if 1 in np.unique(sample_cycle_idx):
                c20off   = pm.Normal('c20off_{}'.format(pb), mu=0, sigma=1)
                c20flag  = True
                n_plot_vars = nvar + 1
            ...
            # run the MCMC
            print("\n\nRunning {}".format(pb))
            trace = pm.sample(cores=4, draws=40000, tune=20000, init='advi+adapt_diag')
    ...
    out['obj'] = objID
    out['obj'] = out['obj'].astype("category")
    out['obj'].cat.set_categories(sorter, inplace=True)
    out.sort_values(['obj'],axis=0, inplace=True)
    objID = out.index.tolist()
